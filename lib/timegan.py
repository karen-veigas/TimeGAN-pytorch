...
class TimeGAN(BaseModel):
    ...
    def backward_g(self):
        """ Backpropagate through netG with realistic return constraints. """

        # === Adversarial ===
        self.err_g_U   = -torch.mean(self.Y_fake)
        self.err_g_U_e = -torch.mean(self.Y_fake_e)

        # === Supervised latent consistency ===
        self.err_s = self.l_mse(self.H_supervise[:, :-1, :], self.H[:, 1:, :])

        # === Return components ===
        log_ret = self.X_hat[..., 1]  # main signal to shape

        # (A) Volatility floor (want std >= 0.03)
        target_vol = 0.03
        vol_per_seq = torch.std(log_ret, dim=1)
        loss_vol = torch.relu(target_vol - torch.mean(vol_per_seq))

        # (B) Negative returns presence (want ≥ 30% negatives)
        target_neg_ratio = 0.30
        neg_ratio = torch.mean((log_ret < 0).float())
        loss_neg = torch.relu(target_neg_ratio - neg_ratio)

        # (C) Drift regularization (keep mean log_ret near 0)
        loss_drift = torch.square(torch.mean(log_ret))

        # === Hard clamp to enforce learnable negatives ===
        log_ret_clamped = torch.clamp(log_ret, -0.10, 0.10)
        clamp_penalty = torch.mean(torch.abs(log_ret - log_ret_clamped))
        loss_clamp = clamp_penalty  # train against saturating positive values
        ...
        self.err_g = (
            λ_adv  * (self.err_g_U + self.err_g_U_e * self.opt.w_gamma)
            + λ_sup * torch.sqrt(self.err_s + 1e-8)
            + λ_vol * loss_vol
            + λ_neg * loss_neg
            + λ_drift * loss_drift
            + λ_clamp * loss_clamp
